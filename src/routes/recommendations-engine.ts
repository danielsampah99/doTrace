...
export async function recommendServices(params: RecommendationParams) {
  ...
  let query = db.select({
    business: businesses,
    distance: sql<number>`|/((${businesses.latitude} - ${latitude})^2 + (${businesses.longitude} - ${longitude})^2)`.as('distance'),
  })
  .from(businesses)
  .where(
    serviceType
      ? eq(businesses.type, serviceType)
      : undefined
  )
  .orderBy(
  ...
  if (userPreferences.serviceTypes?.length) {
    query = query.where(
      or(...userPreferences.serviceTypes.map(t => eq(businesses.type, t)))
    );
  }
