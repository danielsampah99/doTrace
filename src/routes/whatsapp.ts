...
export const twilioRouter = new Elysia({ prefix: '/twilio' })
  .post('/webhook', async ({ body }) => {
    ...
    if (message.toLowerCase().includes('near me')) {
      ...
      const serviceType = message.toLowerCase().replace('near me', '').trim();
      ...
      const messageBody = recommendations.length > 0
